// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot.commands.storage;

import edu.wpi.first.wpilibj.smartdashboard.SmartDashboard;
import edu.wpi.first.wpilibj2.command.CommandBase;
import frc.robot.subsystems.StorageSubsystem;

public class StorageDefault extends CommandBase {

  private final StorageSubsystem m_storage;
  /** Creates a new StorageDefault. */
  public StorageDefault(StorageSubsystem storageSubsystem) {
    m_storage = storageSubsystem;
    addRequirements(storageSubsystem);// Use addRequirements() here to declare subsystem dependencies.
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {}

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    // if (SmartDashboard.getBoolean("Should Use Color Sensor", false)) {
      int measuredBallCount = 0;
      if(m_storage.isTopBlue() || m_storage.isTopRed())
        measuredBallCount++;
      if(m_storage.isBottomBlue() || m_storage.isBottomRed())
        measuredBallCount++;
      // if(measuredBallCount != m_storage.getBallCount())
        // m_storage.setBallCount(measuredBallCount);
        // SmartDashboard.putNumber("color-ball-count", measuredBallCount);
    // }
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(boolean interrupted) {}

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    return false;
  }
}

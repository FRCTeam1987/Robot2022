...
public class WaitForBallShoot extends CommandBase {
  ...
  private int m_desiredBallCount;
  ...
  public WaitForBallShoot(final StorageSubsystem storage, final int desiredBallCount) {
    // Use addRequirements() here to declare subsystem dependencies.
    this(storage, true, desiredBallCount);
  }
  public WaitForBallShoot(final StorageSubsystem storage, final boolean isBallAtTop) {
    // Use addRequirements() here to declare subsystem dependencies.
    this(storage, isBallAtTop, MAX_BALL_COUNT);
  }
  public WaitForBallShoot(final StorageSubsystem storage, final boolean isBallAtTop, final int desiredBallCount) {
    // Use addRequirements() here to declare subsystem dependencies.
    m_storage = storage;
  ...
  public void initialize() {
    ...
    m_desiredBallCount  = Math.min(m_desiredBallCount, m_startBallCount);
  }
  ...
  public boolean isFinished() {
    return m_storage.getBallCount() == 0 ||
      m_storage.getBallCount() == m_startBallCount-m_desiredBallCount;
  }
